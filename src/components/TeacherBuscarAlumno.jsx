...
import {
    Search, AlertCircle, Loader2, UserCircle, Key, Mail,
    ShieldCheck, Calendar, Clock, BookOpen, GraduationCap, Hash
} from 'lucide-react';

const SEARCH_TYPES = [
    { value: 'code',  label: 'Código',              placeholder: 'Ej. 220000000',                              icon: Key  },
    { value: 'email', label: 'Correo electrónico',   placeholder: 'Ej. usuario@udg.mx',                        icon: Mail },
...
export default function TeacherBuscarAlumno({ prefill }) {
    ...
    // Auto-run when a prefill arrives from "Ver detalles"
    ...
    const handleSearch = async (e, overrideType, overrideQuery) => {
        ...
        setResult(null);
        try {
            const response = await api.get(`/teacher/${sType}`, {
                params: { [sType]: sQuery.trim() },
            });
            setResult(response.data);
        } catch (err) {
            if (err.response?.status === 404) {
                const typeName = SEARCH_TYPES.find(t => t.value === sType)?.label?.toLowerCase();
                setError(`No se encontró ningún alumno con ese ${typeName}.`);
            } else {
    ...
    const statusInfo = STATUS_CONFIG[result?.status] || { label: 'Error de consulta', cls: 'bg-gray-100 text-gray-500' };
    ...
    return (
        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden animate-in fade-in duration-300">
            ...
            <div className="bg-gradient-to-r from-[#596E81] to-[#455A6D] px-6 py-4 text-white">
                ...
                <p className="text-xs text-blue-100 mt-0.5">Consulta los datos de un usuario por código, correo o ID</p>
            </div>
                    ...
                    <div className="animate-in fade-in duration-300">
                        ...
                        <div className="bg-gradient-to-r from-[#596E81] to-[#455A6D] rounded-xl p-6 text-white flex items-center gap-4 mb-5">
                            ...
                            <div>
                                ...
                                <div className="flex items-center gap-2 mt-1 text-blue-100 text-sm flex-wrap">
                                    <ShieldCheck size={14} />
                        ...
                        <dl className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
                            {[
                                ...
                                { icon: Calendar,      label: 'Fecha de Registro', value: formatDate(result.createdDate) },
                            ].map(({ icon: Icon, label, value }) => (
                                <div
                                    key={label}
                                    className="border border-gray-100 rounded-xl p-4 bg-gray-50/50 hover:bg-gray-50 hover:shadow-sm transition-all"
                                >
